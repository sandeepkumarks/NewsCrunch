...
class World extends Component {
  ...
  getData = async (limit, lastArticleId) => {
    ...
    let res = await DbService.readData('/news/technology', limit, lastArticleId);
    if(lastArticleId) {
      res.pop();
    }
  ...
  renderItem = ({item}) => {
    return (
      <ListItem thumbnail onPress={() => this.props.navigation.push('NewsDetails', { selectedNews: item, category: 'Technology' })} >
         <Left>
        ...
        <Body style={{paddingHorizontal: 5}}>
          ...
          <Text style={style.fontGrey}>{moment(item.timestamp).local().fromNow()}</Text>
        </Body>
      </ListItem>
    );
  };

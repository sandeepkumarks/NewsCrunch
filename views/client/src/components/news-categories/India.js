...
class World extends Component {
  ...
  getData = async (limit, lastArticleId) => {
    ...
    let res = await DbService.readData('/news/india', limit, lastArticleId);
    if(lastArticleId) {
      res.pop();
    }
  ...
  renderItem = ({item}) => {
    return (
      <ListItem thumbnail onPress={() => this.props.navigation.push('NewsDetails', { selectedNews: item, category: 'India' })} >
        <Left>
